...
describe('Interior Designer App Tests', function () {
  ...
  it('should verify editor UI elements', async function () {
    // 1. Start new project
    await driver.get(`${baseUrl}/`);
    ...
    // Setup
    const nameInput = await driver.wait(until.elementLocated(By.xpath("//input[@placeholder='e.g. Dream Living Room']")), 5000);
    await nameInput.sendKeys(`UI Test ${Date.now()}`);
    const createBtn = await driver.findElement(By.xpath("//button[contains(., 'Create Workspace')]"));
    await createBtn.click();

    // 2. Check Editor Elements
    // Check Top Nav Title
    const TopNav = await driver.wait(until.elementLocated(By.xpath("//h1")), 10000);
    const navText = await TopNav.getText();
    assert.ok(navText.includes('UI Test'), 'Top nav should show design name');

    // Check Sidebar "Library" header
    const libHeader = await driver.findElement(By.xpath("//h3[contains(., 'Library')]"));
    assert.ok(await libHeader.isDisplayed());

    // Check at least one furniture item exists (Lounge Sofa)
    const sofaItem = await driver.findElement(By.xpath("//p[contains(text(), 'Lounge Sofa')]"));
    assert.ok(await sofaItem.isDisplayed());

    // Cleanup: we don't save, just navigate away
    await driver.get(`${baseUrl}/`);
  });
  ...
  it('should handle invalid login attempt', async function () {
    await driver.get(`${baseUrl}/login`);
    const emailInput = await driver.wait(until.elementLocated(By.css('input[type="email"]')), 10000);
    await emailInput.sendKeys(testUser.email);
  ...
  it('should navigate from register page back to login page', async function () {
    await driver.get(`${baseUrl}/register`);
    const signInLink = await driver.wait(until.elementLocated(By.xpath("//a[contains(text(), 'Sign In')]")), 5000);
    await signInLink.click();

    await driver.wait(until.urlIs(`${baseUrl}/login`), 5000);
  });

  it('should redirect to login when accessing dashboard while logged out', async function () {
